<?php
    ...
    if(isset($_POST['buscar'])){
      ...
      if($_POST['ano'] == 'all') {
        $sql = "SELECT curso, COUNT(curso) AS 'evasoes' FROM SolicitacoesEvasao GROUP BY curso";
        $ano = "todos os anos";
      } else {
        $ano = $_POST['ano'];
        $sql = "SELECT curso, COUNT(curso) as 'evasoes' FROM SolicitacoesEvasao WHERE YEAR(data_evasao) = '$ano' GROUP BY curso";
      }
    }
    ...
    if(isset($_POST['buscar'])){
      ...
      if($_POST['ano'] == 'all') {
        $sql2 = "SELECT sexo, COUNT(sexo) AS 'num' FROM SolicitacoesEvasao WHERE sexo='F' UNION SELECT sexo, COUNT(sexo) AS 'num' FROM SolicitacoesEvasao WHERE sexo='M'";
        $ano = "todos os anos";
      } else {
        $ano = $_POST['ano'];
        $sql2 = "SELECT sexo, COUNT(sexo) AS 'num' FROM SolicitacoesEvasao WHERE sexo='F' AND YEAR(data_evasao) = '$ano' UNION SELECT sexo, COUNT(sexo) AS 'num' FROM SolicitacoesEvasao WHERE sexo='M' AND YEAR(data_evasao) = '$ano'";
      }
    }
    ...
    if(isset($_POST['buscar'])){
      ...
      if($_POST['ano'] == 'all') {
        $sql3 = "SELECT motivo_evasao, COUNT(motivo_evasao) AS 'num' FROM SolicitacoesEvasao GROUP BY motivo_evasao";
        $ano = "todos os anos";
      } else {
        $ano = $_POST['ano'];
        $sql3 = "SELECT motivo_evasao, COUNT(motivo_evasao) AS 'num' FROM SolicitacoesEvasao WHERE YEAR(data_evasao) = '$ano' GROUP BY motivo_evasao";
      }
    }
    ...
    if(isset($_POST['buscar'])){
      ...
      if($_POST['ano'] == 'all') {
        $sql4 = "SELECT idade, COUNT(idade) FROM SolicitacoesEvasao GROUP BY idade";
        $ano = "todos os anos";
      } else {
        $ano = $_POST['ano'];
        $sql4 = "SELECT idade, COUNT(idade) FROM SolicitacoesEvasao WHERE YEAR(data_evasao) = '$ano' GROUP BY idade";
      }
    }

    $rs4 = mysqli_query($con, $sql3);
  
    while($dados_4 =  mysqli_fetch_array($rs4)) {
        $dados_idade[] = [strval($dados_4['idade']), intval($dados_4['num'])];
    }
  ...
  <header>
    ...
    <h3>Exibindo dados de <?php echo $ano; ?></h3>
      ...
      <center>
        ...
        <br />
        <center>
          <div id="grafico_cursos"></div>
          <br />
          <div id="grafico_sexo"></div>
          <br />
          <div id="grafico_motivos"></div>
          <br />
          <div id="grafico_idade"></div>
        </center>
      </div>
